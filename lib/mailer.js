		...
		if(err){
			...
			if(transportJSON[appenvironment]){
				...
				if(transportObject.type === 'ses'){
					...
					var originalSendmail = nodemailerTransporter.sendMail;
					nodemailerTransporter.sendMail = function(data,callback){
	...
	async.series({
		...
		getEmailHTML:function(asyncCB){
			if(mailoptions.html){
				...
				if(mailoptions.emailtemplatefilepath && !options.emailtemplatedata.filename){
					options.emailtemplatedata.filename = mailoptions.emailtemplatefilepath;
				}
